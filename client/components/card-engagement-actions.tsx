...
export function CardEngagementActions({ card, sharePath }: CardEngagementActionsProps) {
  ...
  const shareIntroText = useMemo(() => {
    if (card.role === 'developer') {
      return '来分享吧！让更多人知道这位程序员和叩饭（Cofounder），越多人看到，越多人看到，越能帮助更多人匹配到合适的Cofounder。';
    }

    return '来分享吧！让更多人知道这个有趣的项目和叩饭（Cofounder），越多人看到，越能帮助更多人匹配到合适的Cofounder。';
  }, [card.role]);
  ...
  const shareModal =
    shareModalOpen && typeof document !== 'undefined'
      ? createPortal(
          <div aria-label="分享卡片" aria-modal="true" className="fixed inset-0 z-[140] overflow-y-auto p-4" role="dialog">
            <button
              aria-label="关闭转发弹框"
            ...
            <div className="relative z-10 flex min-h-full items-start justify-center py-2 md:items-center">
              <section className="w-full max-w-lg space-y-4 rounded-2xl border border-border/70 bg-card/96 p-5 shadow-[0_18px_42px_rgba(79,108,163,0.24)] backdrop-blur-md">
                ...
                <p className="text-sm text-muted-foreground">{shareIntroText}</p>
